...
- **Agents:** after `git commit`, run `git log -1 --format=%B`. If `Co-authored-by:` appears (often injected by the IDE), amend immediately with the approved message only — see `use skill commit` §5.1. Use `--no-verify` on amend **only** when needed to drop an unauthorized co-author trailer.
