...
**Forma C (O1/O2/O3):** Step 0 memory-bank -> `orchestrate-analyze` -> `orchestrate-deliver` -> `orchestrate-develop`. Skill `memory-bank-init` for manual create/refresh. Guide: [guides/10-forma-c-orquestracao.md](guides/10-forma-c-orquestracao.md). Classic layout: `features/NNN-slug/` (templates under `skills/_shared/templates/features/`).
