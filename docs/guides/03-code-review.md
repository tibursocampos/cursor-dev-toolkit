...
10. Address **Changes required** with `developer`, `sdd-develop`, or manual edits; re-run review if needed.
...
1. **Reviewing without SDD context when it exists** - If you used `sdd-spec` / `sdd-plan` / `sdd-develop`, invoke review **on the same repo** so the agent can find PRD/PLAN. Do not assume artifacts are missing because they are gitignored; they may be under `~/.cursor/sdd/<repo-id>/`.
