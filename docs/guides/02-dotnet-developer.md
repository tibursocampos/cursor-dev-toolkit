...
**`dotnet-developer`** is the toolkit shortcut for **small-to-medium .NET work** without a full SDD cycle (no PRD or PLAN required). You describe the change in chat; the agent loads Clean Architecture and C# guidelines on demand, works on a feature branch, runs `dotnet build` / `dotnet test`, and hands off to `commit` or `code-review`.
...
### Do not use `dotnet-developer` when
...
- You already have an approved **PLAN** → use [01 — SDD workflow](01-sdd-workflow.md): `use skill implement — <plan-path> — Step N`.
...
| Signal | Prefer SDD (`spec` → `plan` → `implement`) |
|--------|---------------------------------------------|
| Database / migrations | Yes |
| Multiple services or repos | Yes |
| New integration or consumer | Yes |
| 10+ files or 4+ hours | Yes |
| Approved PLAN exists | Use `implement`, not `dotnet-developer` |
| Single-area bugfix | **`dotnet-developer`** |
...
```
use skill dotnet-developer — OrderMapper ignores DiscountPercent on export;
...
3. Invoke **`use skill dotnet-developer`** with a concise description and acceptance criteria.
...
Then `use skill plan — <prd-path>` and `use skill implement — <plan-path> — Step 1` per [01 — SDD workflow](01-sdd-workflow.md).
...
```
You: use skill dotnet-developer — CreateUserValidator allows empty email.
...
1. **Skipping PRD on a large feature** — Using `dotnet-developer` for a multi-layer feature or migration leads to incomplete design and huge diffs. When two or more SDD signals apply, switch to [01 — SDD workflow](01-sdd-workflow.md) before coding.
...
3. **Confusing with `fix-build`** — `fix-build` diagnoses **existing** build or test failures (often CI or broken mainline). `dotnet-developer` **implements** a described change. If the repo already fails to compile, invoke `use skill fix-build` first ([05 — operational skills](05-operational-skills.md)).
...
5. **Continuing after scope explosion** — Adding “while you’re here, also add caching and a new endpoint” in the same session. Stop, split work, or escalate to `spec` → `plan` → `implement`.

6. **Using `dotnet-developer` when a PLAN exists** — If you already have `PLAN/PLAN_NNN_*.md`, use `use skill implement — <plan-path> — Step N` so progress stays on the PLAN checklist.
...
| Scope was bigger than expected | [01 — SDD workflow](01-sdd-workflow.md) — `use skill spec` |
