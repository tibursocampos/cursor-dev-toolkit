...
- **SDD** (`spec` → `plan` → `implement`) for medium/high complexity — [01 — SDD workflow](01-sdd-workflow.md).
- **`plan-repo-docs` / `document-repo`** for **this toolkit’s user guides** — those skills document **application repositories** for RAG. The manuals you are reading live in **`docs/guides/`** inside **cursor-dev-toolkit** and are maintained via SDD/implement on that repo (RN04).
- **`fix-build`** when you want to **implement a new feature** — use [02 — dotnet-developer](02-dotnet-developer.md) or SDD instead.
...
**Notes:** May use `gh` for GitHub Actions logs when helpful. Not a substitute for [dotnet-developer](02-dotnet-developer.md) when implementing new behavior.
...
**Typical handoff:** `use skill implement — <plan-path> — Step N` if the change was a PLAN step, or `use skill commit`.
...
**Invoke:** `use skill plan-repo-docs`  
...
**Typical handoff:** `use skill document-repo` (one plan step per session).
...
**Invoke:** `use skill document-repo`

**When to use:** A `docs/documentation-plan/plan.md` already exists in the **target app repo** from `plan-repo-docs`; execute the **next pending** plan step only.

**Typical handoff:** New chat → `use skill document-repo` for the following step, or `use skill commit` when a doc milestone is ready.

**Checkpoint:** One session = one plan step (same discipline as SDD `implement`).
...
**Typical handoff:** `use skill breakdown-tasks` or `use skill spec` to start SDD.
...
**When to use:** After `refine-backlog-item` (or similar input); group implementation steps into `docs/implementation-tasks/<slug>.md` (backend / frontend / tests).

**Typical handoff:** `use skill spec` — paste or reference the refined item and task file.
...
**Typical handoff:** `use skill dotnet-developer` or `use skill implement` for remaining behavior, then `use skill commit`.
...
use skill spec
use skill plan — <prd-path>
use skill implement — <plan-path> — Step 1
```
...
1. **Using `plan-repo-docs` for toolkit guides** — That flow documents **your application** for RAG. **cursor-dev-toolkit** user manuals live in `docs/guides/` and are not created by `plan-repo-docs` in a random app repo.

2. **Multiple `document-repo` steps in one chat** — Same as SDD: one plan step per session keeps progress accurate in `docs/documentation-plan/plan.md`.

3. **Committing on `main` / `develop`** — `use skill commit` enforces branch rules; create `feature/<slug>` first.

4. **Calling `fix-build` for new features** — If the build was green and you need new code, use [dotnet-developer](02-dotnet-developer.md) or SDD, not `fix-build`.
...
| Small .NET change | [02 — dotnet-developer](02-dotnet-developer.md) |
