...
3. **`implement`** — executes **exactly one** PLAN step per chat: code, tests, and PLAN progress update.
...
- You have a **small, isolated .NET fix or refactor** in one area with no PRD needed → use [02 — dotnet-developer](02-dotnet-developer.md) instead.
...
4. **Feature branch** — before `implement`, work on `feature/<slug>` or `feat/<id>`, not `main`, `master`, or `develop`.
...
| Phase | Invoke |
|-------|--------|
| Create PRD | `use skill spec` |
| Create PLAN from PRD | `use skill plan — <prd-path>` |
| Execute one PLAN step | `use skill implement — <plan-path> — Step N` |
...
**Checkpoint rule:** one `implement` invocation = **one** PLAN step. Start a **new chat** for the next step.
...
7. **Handoff:** `use skill plan — <full-prd-path>`
...
2. Type: `use skill plan — <prd-path>` (use the **full** path from the `spec` handoff).
...
6. **Handoff:** `use skill implement — <full-plan-path> — Step 1`

**What the agent will not do in `plan`:** implement code, overwrite your PRD, or run multiple implement steps.

### Phase C — `implement` (one step per session)

1. Check out or create a **valid feature branch**.
2. Open a **fresh chat** (recommended every step, **required** after long sessions).
3. Type: `use skill implement — <plan-path> — Step N` (replace `N` with the pending step number).
...
7. **Next session:** new chat → `use skill implement — <plan-path> — Step N+1`
...
Agent: PRD saved. Handoff: use skill plan — PRD/001_signup_email_validation.md
```

**Chat 2 — plan**

```
You: use skill plan — PRD/001_signup_email_validation.md
...
Agent: PLAN saved (0/4). First step: domain validator + unit tests.
       Handoff: use skill implement — PLAN/PLAN_001_signup_email_validation.md — Step 1
```

**Chat 3 — implement (Step 1 only)**

```
You: use skill implement — PLAN/PLAN_001_signup_email_validation.md — Step 1

Agent: [implements Step 1, updates PLAN to 1/4, marks Step 1 complete]
       Handoff: new chat → use skill implement — PLAN/PLAN_001_signup_email_validation.md — Step 2
```
...
2. **Multiple PLAN steps in one `implement` session** — Each step is sized for one session. Asking for Step 2 in the same chat as Step 1 skips checkpoints, overloads context, and often leaves the PLAN file out of sync. Always open a **new chat** per step (TE02).
...
5. **Ignoring the ~40% context warning** — Long explorations in one chat degrade quality. Save the PLAN update, start fresh, then invoke `implement` with the same plan path and next step number.

6. **Using SDD for a one-file bugfix** — Full SDD overhead is unnecessary for an isolated change. Switch to [dotnet-developer](02-dotnet-developer.md) when the decision tree in [README.md](README.md) points there.
...
| Work was smaller than expected | [02 — dotnet-developer](02-dotnet-developer.md) for the next small task |
| Back to skill map | [Guides README](README.md) |

**Resume SDD:** new chat → `use skill implement — <plan-path> — Step N` for the next pending step.
