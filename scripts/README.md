...
Read-only scan of a **consumer** repo; writes only under `<bank_root>/.inventory/`:

```powershell
.\scripts\inventory\Invoke-MemoryBankInventory.ps1 -RepoPath "D:\Source\Repos\MyApp" -AllowCreateInventory
.\scripts\inventory\Invoke-MemoryBankInventory.ps1 -RepoPath "D:\Source\Repos\MyApp" -BankPath "D:\path\to\global\sdd\MyApp\memory-bank" -AllowCreateInventory
.\scripts\inventory\Invoke-MemoryBankInventory.ps1 -RepoPath "D:\Source\Repos\MyApp" -DryRun
```

`-BankPath` defaults to `<RepoPath>/memory-bank`. For global SDD storage, pass `<classic.path>/memory-bank` (may be outside the consumer repo). Inventory still scans `-RepoPath` (source tree).

Used by `/memory-bank-init`. Memory-bank is **local** (not committed): repository mode uses `/memory-bank/` in the SDD `.gitignore` block; global mode does not edit the consumer `.gitignore`.
