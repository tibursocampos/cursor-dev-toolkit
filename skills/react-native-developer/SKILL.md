...
Use when user asks for `/react-native-developer`, `react-native`, `expo`, or a small isolated React Native / Expo implementation.

## Outcome

Working React Native components and tests in the target workspace, validated with tests/build, with optional handoff to `/commit`.
...
| React guidelines (shared patterns) | `~/.cursor/skills/_shared/react-guidelines/` |
...
Do **not** load `blip-guidelines/` (Blip plugins are web React). Do not preload unrelated guideline trees.
...
Confirm React Native / Expo project (`package.json` with `react-native` and/or `expo`). Read `README.md`, summarize acceptance.

### 1. Guidelines

Load only required shared React/frontend guidelines for this task (navigation, lists, platform APIs as needed).
...
Functional components, hooks, clean React Native architecture. Match existing patterns (Expo Router / React Navigation, StyleSheet, platform splits).

### 5. Tests

Jest + React Native Testing Library (or project equivalents) for changed behavior.
