...
Grouping heuristics and templates for `skills/breakdown-tasks/SKILL.md`. Keep `SKILL.md` under 150 lines.
...
```
use skill spec → use skill plan → use skill implement
```
...
When the handoff table says **PLAN already exists**, resolve the path before suggesting `implement`:
...
Do **not** use `docs/documentation-plan/plan.md` (that is `plan-repo-docs` / `document-repo`, not SDD).
...
| Output | `docs/implementation-tasks/<slug>.md` | `PLAN/PLAN_*.md` or `~/.cursor/sdd/<repo-id>/PLAN/PLAN_*.md` |
